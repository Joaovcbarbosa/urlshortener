...
**                                                       **
** AUTOR: ANTONIO AUGUSTO CHAVES (UNIFESP)               **
***********************************************************/
...
*************************************************************************************/
int Particao(int p, int r, TSolRK Pop[])
{
	TSolRK pivo = Pop[p];
	int topo = p;
...
*************************************************************************************/
void QuickSort(int p, int r, TSolRK Pop[])
{
	int q;	
	if (p < r)
	{
...
void GRASP()
{
  ...
  for (int iter=1; iter <= GRASPmax; iter++)
  {
	...
	printf("\nIter %d: \t Melhor Solucao: %d \t Solucao corrente: %d", iter, sMelhor.fo, sMelhorViz.fo);
  }
  s = sMelhor;
...
void LNS()
{
  ...
  while(Iter-MelhorIter <= LNSmax)
  {
	 ...
	 printf("\nIter: %d \t Melhor solucao: %d \t Solucao corrente: %d ", Iter, sMelhor.fo, sTemp2.fo);
