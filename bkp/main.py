...
def main():
    ...
    for instance in list_of_instance:
        for j in range(rounds):
            ...
            if option == '2':
                MH = 'GRASP'
                result = GRASP(instance, GRASP_max=500, RLC_length_in_percentage=20, alpha_min=1, alpha_max=3, print_solution = True)   
            if option == '3':
                MH = 'ILS'
                result = ILS(instance, betta_min=5, betta_max=30, ILS_max=500, print_solution = True)   
            if option == '4':
                MH = 'VNS'
                result = VNS(instance, r=12, VNS_max=500)     
            if option == '5':
                MH = 'VND'
                result = VND(instance, r=6)     
            if option == '6':
                MH = 'LNS'
                result = LNS(instance, T0=10000, SAMax=50, cooling_rate=0.9, betta_min=5, betta_max=30, RLC_length_in_percentage=20, alpha=0.1, print_solution = True)     
            if option == '7':
                MH = 'BRKGA'
                result = BRKGA(instance, BRKGAMax=1000, p=610, pe_min=10, pe_max=30, rhoe_min=55, rhoe_max=80, pm_min=1, pm_max=7)
            else:
                exit 
            
            if print_solution_option == True: instance.print_solution(result[0], plot_solution_option)
            if print_routes_option == True: instance.print_routes(plot_solution_option)
            if export_results_option == True: export_results(list_of_instance, MH, j, result[0])
